...
if(i >= 0 && i <= 2) {option = i;}
  ...
  public void showMenu(){
   if(opening == true){
    ...
    textSize(fontSize/10);
    textAlign(LEFT);
    text("Space/Enter to select, Up/Down Arrows or W/S to change settings menus, Left/Right Arrows or A/D to change options, TAB to go back.", horizRes/100, 99*vertRes/100);
    textSize(2*fontSize/5);
   }
  }
}
    ...
    if(opening == true){
     ...
     textAlign(CENTER);
      ...
      if(menuSelect == 0){
       ...
       textSize(fontSize/7);
       textAlign(LEFT);
       text("Resolution: Changes the resolution the game\nis displayed at. Goes to 4K, supports typical\n16:9, 16:10, 1:1, and 4:3 resolutions.", 2*horizRes/5, 27*vertRes/40);
       textSize(fontSize/5);
       textAlign(CENTER);
       fill(0);
     }
